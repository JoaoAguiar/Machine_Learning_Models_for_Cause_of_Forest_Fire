...
set.seed(123)
input_train <- createDataPartition(y=fires2015_train$cause_type, p=.7, list=FALSE)
fires_train <- fires2015_train %>% slice(input_train)
fires_test <- fires2015_test
fires_train <- subset(fires_train, select=-c(id, parish, municipality, lat, lon))

bg <- bagging(cause_type ~., fires_train, mfinal=100)
bg_preds <- predict(bg, fires_test)

kaggle_fires <- data.frame(matrix(ncol=0, nrow=3218))
kaggle_fires$id <- fires2015_test$id
kaggle_fires$cause_type <- bg_preds$class
